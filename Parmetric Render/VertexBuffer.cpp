...
VertexBuffer::VertexBuffer(void* list_vertices, UINT size_vertex, UINT size_list,
	...
	RenderChicken* chicken) : m_chicken(chicken) ,m_layout(0), m_buffer(0) {
	...
	if (FAILED(m_chicken->m_d3d_device->CreateBuffer(&buff_desc, &init_data, &m_buffer)))
	{
	...
	if (FAILED(m_chicken->m_d3d_device->CreateInputLayout(layout, size_layout, shader_byte_code, size_byte_shader, &m_layout)))
	{
