...
Texture::Texture(const wchar_t* full_path) : Resource(full_path){
	...
	if (SUCCEEDED(res)) {
		res = DirectX::CreateTexture(GraphicsEn::get()->getRenderChicken()->m_d3d_device, image_data.GetImages(),
		...
		GraphicsEn::get()->getRenderChicken()->m_d3d_device->CreateShaderResourceView(m_texture, &desc,
			&m_shader_res_view);
	}
	else {
		throw std::exception("Texture shat itself");
	}
}
