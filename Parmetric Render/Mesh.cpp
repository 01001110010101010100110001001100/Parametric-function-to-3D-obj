...
Mesh::Mesh(const wchar_t* full_path): Resource(full_path) {
	...
	if (!err.empty())	throw std::exception("Mesh shat itself");
	if (!res)	throw std::exception("Mesh shat itself");
	if (shapes.size() > 1)	throw std::exception("Mesh shat itself");
	...
	GraphicsEn::get()->getVertexMeshLayoutShaderByteCodeAndSize(&shader_byte_code, &size_shader);
	m_vertex_buffer = GraphicsEn::get()->getRenderChicken()->createVertexBuffer(&list_vertices[0], sizeof(VertexMesh),
		(UINT)list_vertices.size(), shader_byte_code, (UINT)size_shader);
	m_index_buffer = GraphicsEn::get()->getRenderChicken()->createIndexBuffer(&list_indices[0], (UINT)list_indices.size());
}
